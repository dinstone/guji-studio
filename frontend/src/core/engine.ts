    ...
    for (var i = 0; i < toks.length; i++) {
      ...
      if (tk.t === 'comment') {
        ...
        while (chars.length > 0) {
          ...
          /* 均衡后仍差 1（奇数格）且长行尾是闭合标点 → 挤挂候选（迷你字形不占格 → 两行齐平底）。
             **仅在真能省一个正文字位时才执行**：挤挂把闭号缩小挪到前字角、牺牲标点形态，
  ...
  function drawColumn(t, m, col, out) {
    ...
    for (var i = 0; i < col.items.length; i++) {
      ...
      var boxH = it.span * m.cellH;
        ...
        var rowCell = Math.min(boxH / maxEff, m.commentCellH);   // 候选（下方被方案A覆盖）
        ...
        rowCell = (maxEff <= 1) ? boxH : (aYN - aY1) / (maxEff - 1);

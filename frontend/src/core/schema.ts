...
export interface SchemaItem {
  ...
  /* roBook：书级作用域下只读（防误清真源），仅单元作用域可编辑。目前只有书名（title_text）用 */
  roBook?: 1
}
...
const gCenter: SchemaGroup = { id: 'center', title: '版心文字（书名 / 卷次 / 页码）', open: 0, items: [
  ...
  { k: 'pager_mode', lb: '页码模式', type: 'seg', opts: [['center', '一叶一码'], ['face', '一叶两码']], tip: '一叶一码 = 版心单列叶号（现状）；一叶两码 = 版心并列竖排两码，右码 = 2n−1（右半叶先读）、左码 = 2n，版心原叶码自动隐藏，目录章首页码随之换成面码' },
